...
from package_publisher_hardening.scanner import (
    DEFAULT_HTML_REPORT,
    DEFAULT_JSON_REPORT,
    scan_repo,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="package-publisher-hardening",
        description="Scan npm and PyPI release workflows for publisher hardening gaps.",
    )
    parser.add_argument("path", help="Repository path to scan.")
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Apply only deterministic workflow OIDC permission patches.",
    )
    parser.add_argument(
        "--report",
